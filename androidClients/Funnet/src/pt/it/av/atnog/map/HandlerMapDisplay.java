...
import pt.it.av.atnog.funNet.cdb.android.Coordinate;
...
public class HandlerMapDisplay extends Handler {
	...
	public void handleMessage(Message msg) {
		Coordinate c = db.get(nome);
		if (firstTime && c != null) {
			map.animateCamera(CameraUpdateFactory.newLatLngZoom(
					new LatLng(c.latitude(), c.longitude()), 18.0f));
			firstTime = false;
		}
		...
		for (int i = 0; i < grupos.size(); i++) {
			marcadores.add(map.addMarker(new MarkerOptions()
					.position(
							new LatLng(db.get(grupos.get(i)).latitude(), db
									.get(grupos.get(i)).longitude()))
					.title(grupos.get(i)).snippet("Texto com algo")
